...
st.title("🎯 YouTube Channel Hunter PRO")import streamlit as st
import requests
from datetime import datetime, timedelta
import pandas as pd
import re
...
# ============================================================
# FACELESS CHANNEL KEYWORDS (Specific Detection)
# ============================================================
FACELESS_NAME_KEYWORDS = [
    # Reddit/Story Channels
    "reddit", "stories", "story", "aita", "askreddit", "confession", "confessions",
    "entitled", "revenge", "petty", "malicious", "nuclear", "pro revenge",
    "relationship", "drama", "update", "saga",
    
    # Horror/Scary
    "horror", "scary", "creepy", "nightmare", "paranormal", "ghost", "haunted",
    "terrifying", "disturbing", "chilling", "spooky", "dark", "creepypasta",
    
    # True Crime
    "true crime", "crime", "murder", "case", "cold case", "unsolved", "mystery",
    "investigation", "detective", "criminal", "killer", "forensic",
    
    # Motivation/Stoicism
    "motivation", "motivational", "stoic", "stoicism", "wisdom", "mindset",
    "discipline", "success", "hustle", "grind", "sigma", "alpha", "masculine",
    "self improvement", "growth", "millionaire", "billionaire",
    
    # Facts/Education
    "facts", "fact", "explained", "education", "educational", "learn", "learning",
    "documentary", "history", "historical", "science", "scientific", "knowledge",
    "interesting", "amazing", "incredible", "mind blowing", "did you know",
    
    # Compilation/Lists
    "compilation", "compilations", "top 10", "top 5", "top 20", "top 100",
    "best of", "worst of", "ranking", "ranked", "countdown", "list",
    "moments", "clips", "highlights", "fails", "wins",
    
    # AI/Automated Indicators
    "ai", "artificial", "generated", "automated", "bot", "voice", "tts",
    "text to speech", "narrator", "narration", "narrated", "voiceover",
    "no face", "faceless", "anonymous", "mystery voice",
    
    # Gaming (Faceless style)
    "gameplay", "walkthrough", "playthrough", "gaming", "gamer", "let's play",
    "no commentary", "longplay", "speedrun", "100%", "all endings",
    
    # Mystery/Conspiracy
    "mystery", "mysteries", "conspiracy", "secret", "secrets", "hidden",
    "unexplained", "strange", "bizarre", "weird", "odd", "unknown",
    
    # Quiz/Trivia
    "quiz", "trivia", "test", "challenge", "guess", "can you",
    
    # Sleep/Relaxation
    "sleep", "relaxing", "relaxation", "calm", "peaceful", "ambient",
    "asmr", "white noise", "rain", "nature sounds", "meditation",
    
    # Cash Cow Indicators
    "cash cow", "passive income", "automation", "autopilot",
]
...
FACELESS_DESC_KEYWORDS = [
    ...
    "reddit stories", "true stories", "real stories", "subscriber stories",
    "voice actor", "narrator", "storytelling", "story time",
    "facts about", "things you", "did you know", "you won't believe",
    "top 10", "top 5", "best of", "worst of", "most",
    "motivational", "inspirational", "life lessons", "wisdom",
    "horror stories", "scary stories", "creepy stories", "true horror",
    "mystery", "unsolved", "unexplained", "strange",
...
NICHE_CATEGORIES = {
    # FACELESS NICHES
    "Reddit Stories": ["reddit", "aita", "askreddit", "tifu", "entitled", "revenge", "confession", "relationship advice"],
    "Horror/Scary": ["horror", "scary", "creepy", "nightmare", "paranormal", "ghost", "haunted", "creepypasta"],
    "True Crime": ["true crime", "crime", "murder", "case", "unsolved", "mystery", "investigation", "killer"],
    "Motivation": ["motivation", "stoic", "stoicism", "mindset", "discipline", "success", "sigma", "wisdom"],
    "Facts/Education": ["facts", "explained", "documentary", "history", "science", "learn", "educational", "knowledge"],
    "Compilation": ["compilation", "best of", "fails", "moments", "highlights", "clips", "funny videos"],
    "Top Lists": ["top 10", "top 5", "ranking", "countdown", "best", "worst", "most"],
    "Mystery": ["mystery", "mysteries", "unsolved", "conspiracy", "secret", "unexplained", "strange"],
    "Quiz/Trivia": ["quiz", "trivia", "test", "challenge", "guess the", "can you"],
    "Sleep/ASMR": ["sleep", "asmr", "relaxing", "calm", "ambient", "meditation", "white noise"],
    
    # GENERAL NICHES
    "News": ["news", "breaking", "headlines", "latest", "update", "report", "current events", "world news", "daily news"],
    "Tech": ["tech", "technology", "gadget", "smartphone", "laptop", "review", "unboxing", "software", "app", "ai"],
    "Gaming": ["gaming", "gameplay", "walkthrough", "gamer", "playthrough", "game", "xbox", "playstation", "nintendo"],
    "Cooking/Food": ["cooking", "recipe", "food", "kitchen", "chef", "meal", "baking", "cuisine", "tasty"],
    "Finance": ["finance", "money", "stock", "crypto", "trading", "invest", "wealth", "business", "economy"],
    "Entertainment": ["entertainment", "celebrity", "movie", "film", "tv show", "drama", "review"],
    "Travel": ["travel", "vlog", "tour", "destination", "explore", "adventure", "trip", "vacation"],
    "Health/Fitness": ["fitness", "health", "workout", "exercise", "gym", "yoga", "weight loss", "diet"],
    "Beauty/Fashion": ["beauty", "makeup", "fashion", "style", "skincare", "tutorial", "haul"],
    "Music": ["music", "song", "cover", "remix", "beat", "lofi", "playlist", "singer"],
    "Sports": ["sports", "football", "basketball", "soccer", "cricket", "nfl", "nba", "highlights"],
    "DIY/Crafts": ["diy", "craft", "handmade", "tutorial", "how to make", "homemade"],
    "Pets/Animals": ["pet", "dog", "cat", "animal", "puppy", "kitten", "wildlife"],
    "Cars/Auto": ["car", "auto", "vehicle", "driving", "motor", "supercar", "review"],
...
def is_faceless_channel(channel_data):
    """Check if channel is likely faceless/automated"""
    ...
    # Bonus for high video count (automation indicator)
...
<html>
<head>
    <title>YouTube Channel Hunter Report - {datetime.now().strftime("%Y-%m-%d")}</title>
    <style>
...
<body>
    ...
    <div class="stats">
        <div class="stat"><div class="num">{len(df)}</div><div class="label">Channels Found</div></div>
        ...
        <div class="stat"><div class="num">{len(df[df['MonetizationScore'] >= 70])}</div><div class="label">Monetized</div></div>
        <div class="stat"><div class="num">${df['EstRevenue'].sum():,.0f}</div><div class="label">Est. Revenue</div></div>
    </div>
"""
    
    for _, r in df.iterrows():
        mon_class = "badge-green" if r['MonetizationScore'] >= 70 else "badge-yellow" if r['MonetizationScore'] >= 50 else ""
    ...
    <div class="card">
        ...
        <div class="metrics">
            ...
            <div class="metric"><div class="val">{r['Subs']:,}</div><div class="lbl">Subscribers</div></div>
            <div class="metric"><div class="val">{r['Virality']:,}/d</div><div class="lbl">Virality</div></div>
            <div class="metric"><div class="val">{r['Engagement']}%</div><div class="lbl">Engagement</div></div>
            <div class="metric"><div class="val">${r['EstRevenue']:,.0f}</div><div class="lbl">Revenue</div></div>
        </div>
        <div>
            <span class="badge {mon_class}">{r['MonetizationStatus']}</span>
            <span class="badge badge-blue">📂 {r['Niche']}</span>
            <span class="badge badge-purple">🔑 {r['Keyword']}</span>
        </div>
        <div class="links">
...
if quota_pct < 50:
    st.sidebar.success(f"🟢 Healthy: {st.session_state['quota_used']:,}/{DAILY_QUOTA_LIMIT:,}")
elif quota_pct < 80:
    st.sidebar.warning(f"🟡 Moderate: {st.session_state['quota_used']:,}/{DAILY_QUOTA_LIMIT:,}")
else:
    st.sidebar.error(f"🔴 Critical: {st.session_state['quota_used']:,}/{DAILY_QUOTA_LIMIT:,}")

st.sidebar.progress(min(quota_pct / 100, 1.0))
st.sidebar.caption(f"Available: {available_quota:,} | Searches left: ~{available_quota // 100}")
...
# ============================================================
# SEARCH MODE SELECTION - MAIN FEATURE
# ============================================================
...
search_mode = st.sidebar.radio(
    ...
    index=2,
    help="Faceless = Reddit, Motivation, Facts etc. | All = News, Tech, Gaming etc."
)

if search_mode == "🤖 Faceless Only":
    st.sidebar.info("✅ Only faceless/automated channels")
elif search_mode == "🌐 All Channels":
    st.sidebar.info("✅ All channels - any niche")
else:
    st.sidebar.info("✅ Prioritize faceless, show all")
...
with st.sidebar.expander("📊 View Filters", expanded=True):
    ...
    min_virality = st.slider("Min Virality (Views/Day)", 0, 5000, 200)

# Subscriber Filters - DEFAULT: 1000
...
with st.sidebar.expander("🎬 Channel Video Count", expanded=False):
    min_videos = st.number_input("Min Videos", min_value=0, value=5, step=5)
    max_videos = st.number_input("Max Videos (0=No Limit)", min_value=0, value=0, step=10)
...
# Region - DEFAULT: Premium ON, Top 5 selected
...
with st.sidebar.expander("🔍 Search Options", expanded=False):
    search_order = st.selectbox("Sort By", ["viewCount", "relevance", "date"], index=0)
...
# Show example based on mode
if search_mode == "🤖 Faceless Only":
    default_kw = """reddit stories
scary stories
motivation
top 10 facts"""
    st.caption("💡 Faceless keywords: reddit, scary, motivation, facts, compilation, top 10")
elif search_mode == "🌐 All Channels":
    default_kw = """news today
tech review
gaming
cooking recipes"""
    st.caption("💡 Any keyword works: news, tech, gaming, cooking, finance, etc.")
else:
    default_kw = """reddit stories
news today
tech review
motivation"""
    st.caption("💡 Mix of faceless + general keywords")
...
# Example Keywords Expander
with st.expander("💡 Keyword Examples for All Niches"):
...
st.markdown(f"📊 **Estimated Quota:** {estimated_quota:,} units | **Available:** {available_quota:,}")

if estimated_quota > available_quota:
    st.warning("⚠️ Not enough quota! Reduce keywords or regions.")
...
if st.button("🚀 SEARCH CHANNELS", type="primary", use_container_width=True):
    ...
    if quota_save and estimated_quota > available_quota:
        ...
        st.warning("🛡️ Quota Save: Reduced to 2 keywords, 2 regions")
    
    st.info(f"🔍 Searching: {len(keywords)} keywords × {len(search_regions)} regions")
    ...
    for kw in keywords:
        ...
        for region in search_regions:
            ...
            status.markdown(f"🔍 `{kw}` | Region: {region}")
